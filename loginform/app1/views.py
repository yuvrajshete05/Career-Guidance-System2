...
#         prompt = (
#             f"Based on the following student details, suggest the top most suitable career paths and explain why each fits the candidate.\n"
#             f"Format your entire response strictly as a JSON object with the following structure:\n"
#             f'{{\n'
#             f'  "recommendation_text": "Your detailed text recommendation here."\n'
#             f'}}\n\n'
...
#     try:
#         genai.configure(api_key="AIzaSyC97fTqvO662tiKiU7nsNizLxoxvm0JKEI")
#         model = genai.GenerativeModel("gemini-1.5-flash") # Or "gemini-pro" for higher quality if needed
#         response = model.generate_content(gemini_prompt)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if result and 'candidates' in result:
                clean_text = result['candidates'][0]['content']['parts'][0]['text'].strip()
                if clean_text.startswith('```json'):
                    clean_text = clean_text[len('```json'):].strip()
                if clean_text.endswith('```'):
                    clean_text = clean_text[:-len('```')].strip()
...
def generate_test(request, test_type): # test_type is correctly received here
    ...
    try:
        genai.configure(api_key=settings.GEMINI_API_KEY)
        model = genai.GenerativeModel("gemini-2.5-flash") # Use gemini-2.5-flash for better performance
        response = model.generate_content(gemini_prompt)
...
# Your Gemini API Key is loaded from settings
API_KEY = settings.GEMINI_API_KEY
